...
void IRGenerator::convertSymbolsToTacky(const Scope &symbols) {
  ...
  for (const auto &[name, entry] : symbols.getSymbols()) {
    ...
    int64_t initValue = 0;
    if (staticAttr->init == InitialValue::Initial &&
        staticAttr->value.has_value()) {
    ...
    if (StaticVars.insert(name.str()).second) {
      ...
      // Re-reading var->getExpr() can miss the initializer when the symbol
      // table decl points to an earlier tentative declaration (e.g. `static int
      // foo;` followed later by `static int foo = 4;`).
      auto *staticVar =
          new ir::StaticVariable(name,               // Name of static var
                                 staticAttr->global, // Is it a global value?
                                 lowerType(var->getType()),
                                 // get the IR type from the variable type
...
ir::Function *
IRGenerator::generateFunction(const FunctionDeclaration &ASTFunc) {
  ...
  for (size_t i = 0, e = nonConstFunc->getArgs().size(); i < e; i++) {
    ...
    // Create ParameterOp with the unique name and lowered IR type
    auto *paramOp =
        new ir::ParameterOp(uniqueName, lowerType(param->getType()));
  ...
  if (ASTFunc.hasBody()) {
    ...
    if (!containsReturn) {
      ir::Constant *RetVal = Ctx.createConstant(Ctx.getInt32Ty(), 0);
...
bool IRGenerator::generateBlockItem(const BlockItem &Item,
                                    ir::Function *IRFunc) {
...
void IRGenerator::generateStatement(const Statement &Stmt,
                                    ir::Function *IRFunc) {
  switch (Stmt.getKind()) {
...
void IRGenerator::generateDeclaration(const VarDeclaration &Decl,
                                      ir::Function *IRFunc) {
  ...
  // (no unique name, no initialization - it refers to a global defined
  // elsewhere)
  ...
  // For static local declarations, use the unique name stored in the
  // declaration Static locals are initialized at compile time (in data
  // segment), not at runtime
  if (Decl.getStorageClass().has_value() &&
...
void IRGenerator::generateReturnStmt(const Statement &Stmt,
                                     ir::Function *IRFunc) {
...
void IRGenerator::generateLabelStmt(const Statement &Stmt,
                                    ir::Function *IRFunc) {
...
void IRGenerator::generateGotoStmt(const Statement &Stmt,
                                   ir::Function *IRFunc) {
...
void IRGenerator::generateCompoundStmt(const Statement &Stmt,
                                       ir::Function *IRFunc) {
...
void IRGenerator::generateWhileStmt(const Statement &Stmt,
                                    ir::Function *IRFunc) {
...
void IRGenerator::generateDoWhileStmt(const Statement &Stmt,
                                      ir::Function *IRFunc) {
  const auto &DoWhile = dynamic_cast<const DoWhileStatement &>(Stmt);

  std::string do_while_start = std::string(DoWhile.get_label()) + "_start";
  std::string do_while_continue =
      std::string(DoWhile.get_label()) + "_continue";
...
void IRGenerator::generateSwitchStmt(const Statement &Stmt,
                                     ir::Function *IRFunc) {
  ...
  for (const auto &caseInfo : cases) {
    ...
    auto *cmp = Ctx.createICmpOp(dynamic_cast<ir::Operand *>(switchValue),
                                 dynamic_cast<ir::Operand *>(caseValue),
                                 ir::ICmpOp::eq, Ctx.getInt32Ty());
...
void IRGenerator::generateCaseStmt(const Statement &Stmt,
                                   ir::Function *IRFunc) const {
...
void IRGenerator::generateDefaultStmt(const Statement &Stmt,
                                      ir::Function *IRFunc) const {
...
ir::Value *IRGenerator::generateExpression(const Expr &Expr,
  ...
  case Expr::Ek_AssignmentOperator:
    return generateAssignmentExpression(
        dynamic_cast<const AssignmentOperator &>(Expr), IRFunc);
  case Expr::Ek_Int:
  case Expr::Ek_IntInit:
  case Expr::Ek_LongInit:
    return generateConstantExpression(Expr);
  case Expr::Ek_UnaryOperator:
    return generateUnaryExpression(dynamic_cast<const UnaryOperator &>(Expr),
                                   IRFunc);
  case Expr::Ek_BinaryOperator:
    return generateBinaryExpression(dynamic_cast<const BinaryOperator &>(Expr),
                                    IRFunc);
  case Expr::Ek_PrefixOperator:
    return generatePrefixExpression(dynamic_cast<const PrefixOperator &>(Expr),
                                    IRFunc);
  case Expr::Ek_PostfixOperator:
    return generatePostfixExpression(
        dynamic_cast<const PostfixOperator &>(Expr), IRFunc);
  case Expr::Ek_ConditionalOperator:
    return generateConditionalExpression(
        dynamic_cast<const ConditionalExpr &>(Expr), IRFunc);
  case Expr::Ek_FunctionCallOperator:
    return generateFunctionCallExpression(
        dynamic_cast<const FunctionCallExpr &>(Expr), IRFunc);
  case Expr::Ek_Cast:
    return generateCastExpression(dynamic_cast<const CastExpr &>(Expr), IRFunc);
  }

  return nullptr; // Should not reach here
}

ir::Value *IRGenerator::generateVarExpression(const Var &var,
                                              ir::Function *IRFunc) {
  ...
  // If the IR name equals the original name, it might be a global/static
  // variable (either because there's no local, or because an extern declaration
  // is in scope)
  if (irName == originalName.str()) {
    // Check if it's a known extern variable (defined in another translation
    // unit)
  ...
  // Static locals have unique names like "functionName.varName" or
  // "functionName.varName.1"
  if (Symbols) {
    if (const SymbolEntry *entry = Symbols->lookupEntry(irName)) {
      if (entry->isStaticAttr()) {
        // Static local - use the unique name from rename stack, type from
        // declaration
        auto *varDecl = std::get<VarDeclaration *>(entry->decl);
  ...
  // getOrCreateVar returns the existing VarOp if already registered by
  // generateDeclaration, so the type fallback here is only used if the variable
  // was somehow never declared.
  ir::Type *ty = var.getType() ? lowerType(var.getType()) : Ctx.getInt32Ty();
  return Ctx.getOrCreateVar(irName, ty);
}

ir::Value *
IRGenerator::generateAssignmentExpression(const AssignmentOperator &assignment,
                                          ir::Function *IRFunc) {
...
ir::Value *
IRGenerator::generateConstantExpression(const Expr &ConstExpr) const {
  switch (ConstExpr.getKind()) {
  case Expr::Ek_Int: {
    auto &constant = llvm::cast<IntegerLiteral>(ConstExpr);
    auto *builtin = llvm::cast<BuiltinType>(constant.getConstantType());
    if (builtin->getBuiltinKind() == BuiltinType::Long)
      return Ctx.createConstant(Ctx.getInt64Ty(),
                                constant.getValue().getSExtValue());
    return Ctx.createConstant(Ctx.getInt32Ty(),
                              constant.getValue().getSExtValue());
  }
  case Expr::Ek_IntInit:
    return Ctx.createConstant(Ctx.getInt32Ty(),
                              llvm::cast<IntInit>(ConstExpr).getValue());
  case Expr::Ek_LongInit:
    return Ctx.createConstant(Ctx.getInt64Ty(),
                              llvm::cast<LongInit>(ConstExpr).getValue());
  default:
    return nullptr;
  }
}

ir::Value *IRGenerator::generateUnaryExpression(const UnaryOperator &UnaryOp,
                                                ir::Function *IRFunc) {
  ...
  ir::UnaryOp *UnOp =
      Ctx.createUnaryOp(dynamic_cast<ir::Operand *>(source), Kind);
  if (IRFunc != nullptr)
    IRFunc->add_instruction(UnOp);
  return UnOp->getDestination();
}

ir::Value *IRGenerator::generateBinaryExpression(const BinaryOperator &BinaryOp,
                                                 ir::Function *IRFunc) {
  ...
  if (Kind != ir::BinaryOp::BinaryOpKind::none) {
    ...
    ir::BinaryOp *BinOp =
        Ctx.createBinaryOp(dynamic_cast<ir::Operand *>(left),
  ...
  if (CmpKind != ir::ICmpOp::CmpOpKind::none) {
    ...
    ir::ICmpOp *CmpOp = Ctx.createICmpOp(dynamic_cast<ir::Operand *>(left),
                                         dynamic_cast<ir::Operand *>(right),
                                         CmpKind, Ctx.getInt32Ty());
    if (IRFunc != nullptr)
  ...
  if (BinaryOp.getOperatorKind() == BinaryOperator::Bok_And) {
    ...
    ir::Type *leftTy =
        (leftOp && leftOp->getType()) ? leftOp->getType() : Ctx.getInt32Ty();
    ...
    if (IRFunc != nullptr) {
      ...
      ir::Type *rightTy = (rightOp && rightOp->getType()) ? rightOp->getType()
                                                          : Ctx.getInt32Ty();
...
ir::Value *IRGenerator::generatePrefixExpression(const PrefixOperator &PrefixOp,
                                                 ir::Function *IRFunc) {
  ir::Value *var = generateExpression(*PrefixOp.getExpr(), IRFunc);
  // Get type from the operand (which was already typed by
  // generateVarExpression/etc.)
  auto *varOp = dynamic_cast<ir::Operand *>(var);
  ir::Type *ty =
      (varOp && varOp->getType()) ? varOp->getType() : Ctx.getInt32Ty();
  auto *one = Ctx.createConstant(ty, 1);

  ir::BinaryOp *increment_instruction = nullptr;

  // For prefix operators, we first update the variable, then return its new
  // value
  ...
  if (IRFunc != nullptr && increment_instruction != nullptr) {
    ...
    IRFunc->add_instruction(
        Ctx.createCopy(increment_instruction->getDestination(), var));
  }
...
ir::Value *
IRGenerator::generatePostfixExpression(const PostfixOperator &PostfixOp,
                                       ir::Function *IRFunc) {
  ...
  // For postfix operators, we save the old value, update the variable, then
...
ir::Value *
IRGenerator::generateConditionalExpression(const ConditionalExpr &cond_expr,
                                           ir::Function *IRFunc) {
  // labels
  auto *e2_label = Ctx.getOrCreateLabel("e2_label");
  auto *end_label = Ctx.getOrCreateLabel("end_label");
  // Result register carries the expression's type (fall back to i32 if not
  // inferred yet)
  ir::Type *condTy =
      cond_expr.getType() ? lowerType(cond_expr.getType()) : Ctx.getInt32Ty();
  ...
  // now we generate the code for the first statement, and obtain the result
  auto *temp1 = generateExpression(*cond_expr.getLeft(), IRFunc);
  IRFunc->add_instruction(Ctx.createMov(temp1, Result));
  IRFunc->add_instruction(Ctx.createJump(end_label));
  // now we generate the second statement
  IRFunc->add_instruction(e2_label);
  auto *temp2 = generateExpression(*cond_expr.getRight(), IRFunc);
...
ir::Value *IRGenerator::generateFunctionCallExpression(
    const FunctionCallExpr &FuncCallExpr, ir::Function *IRFunc) {
  std::vector<ir::Operand *> params;
  for (auto *arg : FuncCallExpr.getArgs()) {
    params.emplace_back(
        reinterpret_cast<ir::Operand *>(generateExpression(*arg, IRFunc)));
  }
  ir::Type *retTy = FuncCallExpr.getType() ? lowerType(FuncCallExpr.getType())
                                           : Ctx.getInt32Ty();
...
ir::Value *IRGenerator::generateCastExpression(const CastExpr &CastExpr,
                                               ir::Function *IRFunc) {
  const auto *expr = CastExpr.getCastedExpression();
  auto *toCastType = CastExpr.getCastedType();

  auto fromRank = BuiltinType::integerRank(
      llvm::cast<BuiltinType>(expr->getType())->getBuiltinKind());
  auto toRank = BuiltinType::integerRank(
      llvm::cast<BuiltinType>(toCastType)->getBuiltinKind());

  auto *src = dynamic_cast<ir::Operand *>(generateExpression(*expr, IRFunc));

  if (toRank == fromRank) {
    return src; // no-op cast
  }

  if (toRank > fromRank) {
    auto *se = Ctx.createSignExtend(src, lowerType(toCastType));
    IRFunc->add_instruction(se);
    return se->getResult();
  } else {
    auto *tru = Ctx.createTruncate(src, lowerType(toCastType));
    IRFunc->add_instruction(tru);
    return tru->getResult();
  }
}

void IRGenerator::collectSwitchCases(const Statement *stmt,
                                     std::vector<CaseInfo> &cases,
                                     std::string &defaultLabel,
                                     bool &hasDefault) {
  if (!stmt)
  ...
  case Statement::SK_Compound: {
    ...
    for (const auto &item : *compound) {
      if (std::holds_alternative<Statement *>(item)) {
        collectSwitchCases(std::get<Statement *>(item), cases, defaultLabel,
                           hasDefault);
      }
    }
    break;
